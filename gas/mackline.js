...
//     ユーロドル 237件 勝率34% PF0.88   -907  年 -327/-303/-462/+185  ← 単体では負け
//     ドルカナダ 269件 勝率38% PF0.97   -595  年  -89/-250/-744/+489  ← 単体では負け
//     ※ユーロドル・ドルカナダは単体マイナスだが、サインだけで機械的に入るのではなく
//       裁量で選ぶ前提なので、参考情報として6通貨すべて通知する（ユーザー指示 2026-08-28）
...
// ★2026-09-03 ユーザー指示：3方式ともドル円のみ通知する
//   逆張り単体の成績はユーロ円+1,885 / ポンドドル+1,035 の方が良いが、
//   通知過多を避けてドル円(+677)に統一。戻す時は下の1行に通貨を足すだけ
var REV_PAIRS = ['USD/JPY'];
  ...
  if (arm) {
    ...
    try {
      pushMail_('🔄 1時間足の転換｜' + name + '　' + (arm.dir === 'up' ? '買い' : '売り') + '　高安値を更新',
        ...
        '検証(2023-01〜2026-07・6通貨計)：勝率34〜45%・+2,725pips。利確は損切り幅の1.5倍。ユーロドルとドルカナダは単体では負けるので参考扱い。\n' +
